...
$(document).ready(function() {
    ...
    function configureForWidth(table, columns) {
        if($(window).width() <= 899 && $(window).width() > 499) {
        ...
        } else if($(window).width() <= 499 && $(window).width() > 0) {
            $.each(columns, function(index, priority) {
    ...
    var settings = {
        ...
        'aoColumns': [{
            ...
            'mData': function(source) {
                var gainPercentage =
                    (((source.lastTrade * source.quantity) - (source.pricePaid * source.quantity)) / (source.pricePaid * source.quantity)) * 100;
        ...
        'sScrollY': getHeight(),
        //set vertical scroll, with height
        'bPaginate': false,
        //toggle pagination
        'bInfo': false,
        //toggle dataset info at bottom of table
        'bDestroy': true,
        //toggle allow replacing of table
        'bSort': true,
        //toggle sorting
    ...
    function configureTable(settings) {
        ...
        var table = $('#positions-table').dataTable(settings); //replace table with new settings
